...
assert.equal(mod.name, 'dsh-coverage-tracker', 'plugin name export')
...
const registered = []
mod.apply({ tools: { register: def => registered.push(def) } }, { defaultMinLines: 80, highThreshold: 80, lowThreshold: 50 })
assert.deepEqual(
  registered.map(t => t.name).sort(),
  ['coverage_check', 'coverage_heatmap', 'coverage_parse'],
